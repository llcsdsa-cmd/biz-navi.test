...
==========================================================
  ...
  コア機能（全機能）       : ██████████████████  約 95% 完成
  取引入力UI（新設計§6）  : ████████████████░░  約 85% 完成  ←3ステップUI実装済み／numeric強制・Undo未実装
  UI・UX・レスポンシブ     : ██████████████████  約 93% 完成
...
【高優先度】
  ...
  🔧 §6 入力UIの品質強化
     金額欄を inputmode="numeric" + pattern="[0-9]*" で
     全角入力を受け付けない半角テンキー固定にする
     按分率計算後の金額は Math.round() で整数化（1円未満四捨五入）
     登録直後のトーストに「↩️ 取り消す」を5秒間表示するUndo機能
     （誤タップ時は訂正ログを残さず物理削除・入力値を維持して引き戻し）
  🔧 §6 カメラ・写真管理の強化
